<template>
  <div class="page">
    ...
    <el-card class="box-card">
      ...
      <!-- 用户列表区 -->
      <el-table :data="userInfo" style="width: 100%" border stripe>
      ...
      >
        <div>
          <p>当前的用户：{{userList.username}}</p>
          <p>当前的角色：{{userList.role_name}}</p>
          <p>分配新角色：
...
import { getRolesList } from "../../api/power";
export default {
  components: {},
  data() {
    //   验证邮箱的规则
  ...
  methods: {
    ...
    async saveRoleInfo() {
        ...
        console.log('roleID',this.userList.id)
        const {data: res} = await putSaveRidInfo(this.userList.id, this.selectedRoleId)
        console.log('分配角色请求',res)
        if(res.meta.status !== 200) {
